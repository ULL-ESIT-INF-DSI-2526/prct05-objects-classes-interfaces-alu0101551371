...
import {
  ...
  TFG,
  TFM,
} from "../src/ej1_gestor_referencias_bibliográficas/elementos";
...
describe("Gestor de referencias bibliográficas - Elementos Bibliográficos", () => {
  ...
  test("Interfaz elementos de información", () => {
    ...
    expect(informacion.titulo).toBe(
      "DIGNEA: A tool to generate diverse and discriminatory instance suites for optimisation domains",
    );
    expect(informacion.autor).toEqual([
      "Alejandro Marrero",
      "Eduardo Segredo",
      "Coromoto León",
      "Emma Hart",
    ]);
    expect(informacion.palabrasClave).toEqual([
      "instance generation",
      "novelty search",
      "evolutionary algorithm",
      "knapsack",
    ]);
    expect(informacion.resumen).toBe(
      "Framework generalizable para generar instancias diversas y discriminatorias en dominios de optimización.",
    );
  ...
  test("Formato IEEE", () => {
    expect(revista1.getFormatoIEEE()).toBe(
      'Alejandro Marrero, Eduardo Segredo, Coromoto León, Emma Hart, "DIGNEA: A tool to generate diverse and discriminatory instance suites for optimisation domains," SoftwareX, 2023. Vol. 22, No. 0.',
    );
    expect(revista2.getFormatoIEEE()).toBe(
      'Rafael Herrero-Álvarez, Gara Miranda, Coromoto León, Eduardo Segredo, "Engaging Primary and Secondary School Students in Computer Science Through Computational Thinking Training," IEEE Xplore, 2022. Vol. 1, No. 9749967.',
    );
    expect(congreso1.getFormatoIEEE()).toBe(
      'Alejandro Marrero, Eduardo Segredo, Emma Hart, Jakob Bossek, Aneta Neumann, "Generating diverse and discriminatory knapsack instances by searching for novelty in variable dimensions of feature-space," ACM, 2023. presented at GECCO \'23, Lisbon, Portugal.',
    );
    expect(congreso2.getFormatoIEEE()).toBe(
      'Francisco Cruz-Zelante, Eduardo Segredo, Gara Miranda, "Dealing with a problematic roundabout by optimizing a traffic light system through evolutionary computation," ACM, 2021. presented at GECCO \'21 Companion, Lille, France.',
    );
    expect(libro.getFormatoIEEE()).toBe(
      'Alejandro Marrero, Eduardo Segredo, Coromoto León, Emma Hart, "A Novelty-Search Approach to Filling an Instance-Space with Diverse and Discriminatory Instances for the Knapsack Problem," Springer International Publishing, 2022. ISBN: 978-3-031-14714-2.',
    );
    expect(tfg.getFormatoIEEE()).toBe(
      'Cristo Daniel Navarro Rodríguez, "Resolución del Multi Depot Cumulative Capacitated Vehicle Routing Problem mediante computación evolutiva.," Universidad de La Laguna, 2021. Tutor: No disponible.',
    );
    expect(tfm.getFormatoIEEE()).toBe(
      'Alejandro Marrero Díaz, "On the automatic planning of healthy and balanced menus," Universidad de La Laguna, 2019. Tutor: No disponible.',
    );
  });
});
...
describe("Gestor de referencias bibliográficas - Inventario", () => {
  ...
  test("Filtrado por palabra clave", () => {
    inventario.setInventario([revista, congreso]);
    expect(inventario.filtrado({ keywords: "knapsack" })).toEqual([
      revista,
      congreso,
    ]);
  });
  ...
  test("Filtrado por fecha de publicación", () => {
    inventario.setInventario([revista, congreso]);
    expect(
      inventario.filtrado({ publishDate: new Date("2023-05-01") }),
    ).toEqual([revista]);
  });
  ...
  test("Should return an empty array if no elements match the filter", () => {
    ...
    expect(
      inventario.filtrado({ publishDate: new Date("2000-01-01") }),
    ).toEqual([]);
    expect(
      inventario.filtrado({ editorial: "Non-existent editorial" }),
    ).toEqual([]);
  });
  ...
  test("Filtrado con múltiples criterios", () => {
    ...
    ).toEqual([revista, congreso]);
    expect(
      inventario.filtrado({ keywords: "knapsack", author: "Jakob Bossek" }),
    ).toEqual([congreso]);
    expect(
      inventario.filtrado({
        keywords: "knapsack",
