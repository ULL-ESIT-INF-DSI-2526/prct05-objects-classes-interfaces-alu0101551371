...
export class Inventario {
  ...
  public filtrado(filtro: {
    ...
    new_inventario = new_inventario.filter((elemento) => {
      ...
      if (filtro.keywords && !elemento.palabrasClave.includes(filtro.keywords))
        return false;
      if (filtro.author && !elemento.autor.includes(filtro.author))
        return false;
      if (
        filtro.publishDate &&
        elemento.fechaPublicacion.getTime() != filtro.publishDate.getTime()
      )
        return false;
      if (filtro.editorial && elemento.editorial != filtro.editorial)
        return false;
      return true;
    });
